...
const TutorialManager = ({
  ...
  useFrame(() => {
    if (!playerPosition || !amyPosition || !doorPosition) return;

    const playerAmyDistance = checkDistance(playerPosition, amyPosition);
    const playerDoorDistance = checkDistance(playerPosition, doorPosition);
    const amyDoorDistance = checkDistance(amyPosition, doorPosition);

    // Update debug info
    setDebugInfo({
      playerToAmy: playerAmyDistance.toFixed(2),
      playerToDoor: playerDoorDistance.toFixed(2),
      amyToDoor: amyDoorDistance.toFixed(2)
    });
    ...
    if (tutorialStep === 1) {
      ...
      if (playerDoorDistance < 4 && amyDoorDistance < 4) {
